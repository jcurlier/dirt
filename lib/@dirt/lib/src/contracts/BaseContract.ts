...
export abstract class BaseContract implements IContract {
    ...
    constructor(public context: Dirt, protected config: IContractConfiguration) {
    ...
    protected async dispatchTransaction(func: string, ...args: any[]) {
    ...
    private addErrorSourceAddress(address: string): void {
        this.errorAddresses.add(address);
    }

    private async buildError(name: string, ex: Error): Promise<ContractDispatchError> {
        let block = this.context.web3.eth.getBlock(this.context.web3.eth.blockNumber, true);
