import { IContractConfiguration, IContract } from "./IContract";
...
export class Token extends BaseContract {
    ...
    async transfer(toAddress: string, amount: TokenValue): Promise<void> {
        ...
        await this.instance.transfer(toAddress, amount.raw, { from: this.context.defaultAccount() });
    }

    async balanceOf(account?: string): Promise<TokenValue> {
        account = account || this.context.defaultAccount();
        let raw = await this.instance.balanceOf.call(account, { from: this.context.defaultAccount() });
    ...
    async approveFor(spender: string, value: TokenValue): Promise<void> {
        ...
        await this.instance.approve(spender, value.raw, { from: this.context.defaultAccount() });
    }
    ...
    async increaseApproval(spender: string, value: TokenValue): Promise<void> {
        let currentAllowance = await this.allowance(this.context.defaultAccount(), spender);
        ...
        await this.instance.approve(spender, increased.raw, { from: this.context.defaultAccount() });
    }
    ...
    async decreaseApproval(spender: string, value: TokenValue): Promise<void> {
        ...
        await this.instance.approve(spender, decreased.raw, { from: this.context.defaultAccount() });
    }

    async ensureSpenderApproval(spender: string, required: TokenValue): Promise<TokenSpenderApprovalScope> {
        let current = await this.allowance(this.context.defaultAccount(), spender);
